...
/**
 * <p>The API server that is responsible for providing a secure HTTPS connection on a random
 * available port using a closed-system certificate. The self-signed certificate is generated
 * for each instance of the FuseAPIServer and is never cached or re-used. This means on every
 * application launch, a new keypair is generated and held in memory. The webview is configured
 * to only accept the SSL certificate that is known to be generated by itself.</p>
 * <p>This self-signed certificates, due to the closed-system nature provides enough trust that data cannot be spied upon during transfer between the webview and
 * the native environment</p>
 * <p>To avoid clashing with other applications the API server will claim any available port. Use {@link #getPort()}
 * to find which port the OS has allowed us to consume.</p>
 * <p>The API server also expects that all requests to provide a secret key which is cryptographically generated. Use {@link #getSecretKey()}
 * which must be passed back to the API server in a HTTP header, "X-Fuse-Secret". Connections without this header with the expected value
 * will be closed immediately.</p>
 * <p>Like the TLS certificates, the API secret is generated on each instance, and in the future will rotate. The application should not cache this value.</p>
 * <p>The application <strong>MUST NOT</strong> disclose the API port or the API secret, as that will allow unauthorized access to make API calls on the behalf of the application</p>
 */
public class FuseAPIServer {
    ...
    /**
     * Gets the API secret that must be used in the X-Fuse-Secret header on API requests.
     * This secret is generated on every instance creation and in the future will be short-lived
     * and regularly rolled. Do not cache this value.
    ...
    /**
     * The port that the API server is listening on.
    ...
    /**
     * Verifies that the SSL certificate was signed with our generated certificates.
     * @param certificate the request certificate
     * @return true if certificate can be verified
     */
