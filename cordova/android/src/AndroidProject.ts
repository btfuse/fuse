...
export class AndroidProject {
    ...
    public constructor(config: ConfigParser) {
        this.$config = config;
    ...
    private async $writeAppResourcs(destination: string): Promise<void> {
    ...
    public async create(dir: string): Promise<void> {
        ...
        await Promise.all([
            ...
            this.$writeAppResourcs(destination)
        ]);
    }
}
